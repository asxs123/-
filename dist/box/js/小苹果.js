var rule = {
    ...
    二级: $js.toString(() => {
        ...
        const data2 = data.type_id_1; // 1/2/3/4 四种类型
        const urls = data.urls;
        let d = [];
        let keyMap = {};
        let maxNum = 0;
        // 核心：根据data2的4种类型，做极致性能的分支处理
        switch (data2) {
            // ✅ 场景1：data2=1/3 → key为文本，仅过滤「及时雨」，无需排序，原顺序保留
            case 1:
            case 3:
                urls.forEach(item => {
                    if (item.key !== '及时雨') { // 只过滤及时雨，其他文本全部保留
                        d.push(`${item.key}$http://c.xpgtv.net/m3u8/${item.url}.m3u8`);
                    }
                });
                break;
            // ✅ 场景2：data2=2/4 → key为纯数字，过滤及时雨+数字升序，极致性能写法（核心最优）
            case 2:
            case 4:
                urls.forEach(item => {
                    const numKey = Number(item.key);
                    if (!isNaN(numKey)) { // 自动过滤「及时雨(NaN)」+ 所有非数字key
                        keyMap[numKey] = item;
                        maxNum = numKey > maxNum ? numKey : maxNum;
                    }
                });
                // 数字遍历天然升序，无排序开销
                for (let i = 1; i <= maxNum; i++) {
                    keyMap[i] && d.push(`${i}$http://c.xpgtv.net/m3u8/${keyMap[i].url}.m3u8`);
                }
                break;
        }
        VOD = {
