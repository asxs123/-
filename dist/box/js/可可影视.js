var rule = {
  ...
  headers: {
    ...
    Referer: HOST + "/",
    "X-Forwarded-For": `119.${Math.floor(Math.random() * 254) + 1}.${
  ...
  预处理: $js.toString(() => {
    ...
    if (hash && hash !== getItem("myhash")) {
      ...
      for (let i = 0; i < 1000000; i++) {
        ...
        if (
          sha1.charCodeAt(idx) === 0xb0 &&
          sha1.charCodeAt(idx + 1) === 0x0b
        ) {
