...
async function home(filter) {
    ...
    const excludeTypes = filter?.exclude || ['短剧'];
    const filteredCategoryList = categoryList.filter(item => !excludeTypes.includes(item.name));
    const tasks = filteredCategoryList.map(item => fetchFilterTask(item));
